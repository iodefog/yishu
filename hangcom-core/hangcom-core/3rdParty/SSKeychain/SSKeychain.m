...
+ (NSString *)passwordForService:(NSString *)service account:(NSString *)account {
    NSError *error;
    NSString *value = [self passwordForService:service account:account error:&error];
    if (error) {
         NSLog(@"%@", error);
    }
	return value;
...
+ (BOOL)setPassword:(NSString *)password forService:(NSString *)service account:(NSString *)account {
    NSError *error;
    BOOL tf = [self setPassword:password forService:service account:account error:&error];
    if (error) {
        NSLog(@"%@", error);
    }
    return tf;
