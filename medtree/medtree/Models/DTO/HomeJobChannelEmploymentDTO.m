...
@implementation HomeJobChannelEmploymentDTO
...
- (BOOL)parse:(NSDictionary *)dict
{
    ...
    if (natureArray.count > [self getIntValue:dict[@"enterprise_type"]]) {
        self.enterpriseNature = natureArray[[self getIntValue:dict[@"enterpriese_type"]]];
    }
    ...
    if (levelArray.count > [self getIntValue:dict[@"enterprise_level"]]) {
        self.enterpriseLevel = levelArray[[self getIntValue:dict[@"enterpriese_level"]]];
    }
    ...
    if (sizeArray.count > [self getIntValue:dict[@"enterprise_scale"]]) {
        self.enterpriseSize = sizeArray[[self getIntValue:dict[@"enterpriese_scale"]]];
    }
    
    self.publishTime = [DateUtil getDisplayTime:[self getDateValue:[dict objectForKey:@"publish_time"]]];
