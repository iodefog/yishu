...
{
    self.infiniteScrollingView.showNoMoreLogo = showNoMoreLogo;
}

- (BOOL)showNoMoreLogo
{
    return self.infiniteScrollingView.showNoMoreLogo;
}
...
- (void)setState:(SVInfiniteScrollingState)newState {
    ...
    if(hasCustomView) {
        ...
        [self.noMoreLabel setFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
        switch (newState) {
    ...
    self.activityIndicatorView.hidden = self.showNoMoreLogo;
    self.noMoreLabel.hidden = !self.showNoMoreLogo;
    if (self.showNoMoreLogo) {
        return;
    }
